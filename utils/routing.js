...
class SmartRouter {
    static analyzeAndRoute(query, forceModel) {
        const queryLower = query.toLowerCase();
        const keywords = this.extractKeywords(queryLower);
        if (forceModel && models_js_1.GOOGLE_MODELS[forceModel]) {
            return {
                selectedModel: forceModel,
                taskType: models_js_1.GOOGLE_MODELS[forceModel].strength,
                reasoning: `Modell wurde explizit angefordert: ${forceModel}`,
                confidence: 1.0
            };
        }
        const scores = {};
        Object.keys(models_js_1.GOOGLE_MODELS).forEach(model => {
            scores[model] = 0;
        });
        let primaryTaskType = "text";
        let maxScore = 0;
        if (keywords.video > 0) {
    ...
    static generateReasoning(model, keywords, score) {
